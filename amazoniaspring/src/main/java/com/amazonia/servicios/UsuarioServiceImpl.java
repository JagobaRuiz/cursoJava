...
public class UsuarioServiceImpl extends AnonimoServiceImpl implements UsuarioService {
	...
	public Factura facturar(Cliente cliente, Carrito carrito) {
		...
		carrito.getLineas().stream().forEach(lineaCarrito -> {
			var lineaFactura = Factura.Linea.builder().factura(factura).nombre(lineaCarrito.getProducto()
					.getNombre()).precio(lineaCarrito.getProducto().getPrecio()).cantidad(lineaCarrito.getCantidad())
					.producto(lineaCarrito.getProducto()).build();
